...
namespace Radar.Utilities
{
	public static class Radar
	{
		...
		public static void RefreshScreen(Control screen, ToolTip tp)
		{
			...
			{
				var drawThread = new Thread(delegate()
				                            {
				                            	DrawWowBlip(new WowGOBlip(wowGameObject), screen, tp);
				                            });
				// Ensure that any blip representing the given locked WowObject type is not being drawn twice at the
				// same time.
				lock(wowGameObject)
					drawThread.Start();
			}
			...
			foreach (var unit in ObjectManager.GetObjectsOfType<WowUnit>(true, true))
			{
				...
				//	Ensure that any blip representing [unit] is not being drawn in multiple threads at the same time.
				lock(unit)
					drawThread.Start();
